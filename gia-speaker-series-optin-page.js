/* GIVE IT ALL - Speaker Series OPT-IN / teaser page. Tag: gia-speaker-series-optin-page. All videos locked; register (GHL) to unlock. Turn Wix header+footer OFF. */
(function(){
  if (customElements.get('gia-speaker-series-optin-page')) return;
  var CSS = `
  :host{
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:600px){
    ...
    .reg-head h3{font-size:1.15rem}
    .reg-head p{font-size:.85rem}
  ...
  /* registration modal */
  .reg-modal{position:fixed;inset:0;z-index:1000;display:none;align-items:center;justify-content:center;padding:20px}
  .reg-modal.on{display:flex}
  .reg-backdrop{position:absolute;inset:0;background:rgba(4,18,42,.78);backdrop-filter:blur(6px)}
  .reg-card{position:relative;z-index:1;width:100%;max-width:520px;max-height:92vh;overflow:auto;background:#1A1A1A;border:1px solid rgba(232,198,95,.25);border-radius:var(--r-card);padding:28px clamp(20px,3vw,32px) 24px;box-shadow:0 40px 90px -30px rgba(0,0,0,.8)}
  .reg-close{position:absolute;top:12px;right:14px;background:none;border:none;color:var(--steel);font-size:1.7rem;line-height:1;cursor:pointer;transition:color .2s}
  .reg-close:hover{color:var(--ink)}
  .reg-head{text-align:center;margin-bottom:16px}
  .reg-head .eyebrow{display:block;margin-bottom:6px}
  .reg-head h3{color:var(--ink);font-size:clamp(1.3rem,2.4vw,1.7rem);font-weight:800;line-height:1.15}
  .reg-head p{color:var(--steel);font-size:.92rem;margin-top:8px}
  .reg-form{min-height:460px}
  .reg-form iframe{display:block;width:100%}
  `;
      ...
      <div class="series-grid">
        <aside class="rail">
          <div class="rail-top">
            ...
            <span class="rail-progress">Locked
              <span class="dots"><i></i><i></i><i></i><i></i><i></i></span>
            </span>
          </div>

          <div class="plist" id="plist">
            <div class="pitem locked active" data-key="intro">
              <div class="pthumb">
                ...
                <div class="glass"><svg class="lockico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="11" width="16" height="9" rx="2"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/></svg></div>
              </div>
              <div class="pmeta">
                <div class="pnum">Module 0 &middot; Introduction</div>
                <div class="ptitle">Your Story Matters</div>
                <div class="pdur">Free with sign-up</div>
              </div>
            </div>
        ...
        <div class="player-card">
          <div class="player" id="player">
            <span class="free-tag">Free with sign-up</span>
            <div class="poster" id="poster">
              <img class="thumb" id="posterImg" src="https://static.wixstatic.com/media/111174_d9640c4c66904567a634f847ec653bb1~mv2.png" alt="Your Story Matters">
            </div>
  ...
  <div class="reg-modal" id="regModal" aria-hidden="true">
    ...
    <div class="reg-card" role="dialog" aria-modal="true" aria-label="Register to watch the free intro">
      ...
      <div class="reg-form">
        ...
        <iframe id="ghl-form" src="https://api.leadconnectorhq.com/widget/form/d3GUb7jCQSFPapYJ824m"
          title="Register" scrolling="no" style="width:100%;height:100%;min-height:460px;border:none;background:transparent"></iframe>
      </div>
    </div>
  </div>
  ...
  // GoHighLevel form embed helper (auto-resizes the GHL iframe)
  if (!document.getElementById('ghl-embed-js')) { var g=document.createElement('script'); g.id='ghl-embed-js'; g.src='https://link.msgsndr.com/js/form_embed.js'; document.head.appendChild(g); }

  class GIASpeakerSeriesOptinPage extends HTMLElement {
    connectedCallback(){
      ...
      var modules={
        ...
        final:{num:'Closing &middot; Go Even Further',title:'One Last Thing',summary:'You\'re ready. If you want to take it even further, here\'s what\'s available to you.',thumb:'https://static.wixstatic.com/media/111174_dfeaac73286341ddbb051d729434f7c2~mv2.png'}
      };
      var player=root.getElementById('player'), posterImg=root.getElementById('posterImg');
      var lockTitle=root.getElementById('lockTitle');
      ...
      root.querySelectorAll('.pitem').forEach(function(item){
        item.addEventListener('click',function(){
          ...
          pbNum.innerHTML=m.num; pbTitle.innerHTML=m.title; pbDesc.textContent=m.summary;
          if(posterImg && m.thumb){ posterImg.src=m.thumb; posterImg.alt=m.title; }
          if(lockTitle){ lockTitle.textContent=(key==='intro'?'Register to watch the intro':m.title+' is locked'); }
          player.scrollIntoView({behavior:'smooth',block:'center'});
        });
      });

      // registration modal (GHL form) + open/close
      var regModal=root.getElementById('regModal');
      function openReg(){ regModal.classList.add('on'); regModal.setAttribute('aria-hidden','false'); document.body.style.overflow='hidden'; }
      function closeReg(){ regModal.classList.remove('on'); regModal.setAttribute('aria-hidden','true'); document.body.style.overflow=''; }
      root.querySelectorAll('.js-reg').forEach(function(b){ b.addEventListener('click',openReg); });
      root.querySelectorAll('[data-close]').forEach(function(b){ b.addEventListener('click',closeReg); });
      document.addEventListener('keydown',function(e){ if(e.key==='Escape') closeReg(); });
    }
  }
  customElements.define('gia-speaker-series-optin-page', GIASpeakerSeriesOptinPage);
})();
